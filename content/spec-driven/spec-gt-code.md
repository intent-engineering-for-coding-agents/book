...
This is not a thought experiment. Teams using agentic tools have done exactly this: deleted an implementation that was tangled, unmaintainable, accreted over years of hotfixes, and replaced it in a session from the canonical spec. The new implementation passed the same tests. The feature set was identical. The code was cleaner.
...
Code is generated. Specs are authored. Code is output. Specs are input.
...
In the agentic era, code occupies the same position that the compiled binary used to. It is the output of a process. The spec, the acceptance criteria, the intent: that is the source. Agents regenerate the output from the source. They do not regenerate the source from the output.

The practical consequence: when there is a conflict between the spec and the code, the spec is right. When the spec needs to change, change the spec and update the code to match. When the code is tangled beyond easy modification, regenerate it from the spec.
...
Before agentic tools, modifying code was expensive. Writing a spec and then implementing from it doubled the work. Code became the source of truth because it was the hard part. Documentation was aspiration.
...
Code modification is not expensive anymore. A session that regenerates a service takes an afternoon. A session that regenerates a service without a spec takes an afternoon and produces something the next developer cannot extend without reverse-engineering the intent. Code that takes an afternoon to generate should not be treated as more valuable than the spec that makes it reproducible.
...
This is not a new insight. What is new is that agentic speed makes the cost of ignoring it visible. A team working at AI velocity without spec discipline produces more broken things faster. The spec is not overhead. It is the mechanism that makes speed sustainable.
...
This is the practical demonstration of the thesis. When the result is wrong, the code is what you discard. Improve the spec based on what you learned, and regenerate. The spec accumulates understanding with each iteration; the code is a snapshot.
...
Stop treating code review as the primary quality gate. Start treating spec review as the primary quality gate. If the spec is correct, the code is likely to be correct. If the spec is wrong, no amount of code review will catch the implementation of the wrong thing. Review the intent first. Then the diff.

This claim holds up only if the spec is connected to something harder than intent. Not a document that describes the expected behaviour. Executable proof that the implementation actually delivers it. Not a human scanning the diff. Tests that run in CI and fail when the implementation diverges from the spec. Intent without proof is still just a document.
