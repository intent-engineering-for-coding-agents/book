...
The team is not lying. They believe they practice it. They probably did practice it for the first month after adoption. The gap between what we believe we do and what the repo shows we do is the most reliable failure mode in any engineering organization, and Intent Engineering practices are no different.
...
Honest labeling produces two outcomes. First, it tells everyone what they rely on. CI-enforced means trust the pipeline. Practiced means ask someone who knows. Second, it makes gaps visible, which makes them closeable. A practice marked practiced, target: CI-enforced by Q3 is a backlog item. A practice marked CI-enforced but caught only in code review is an outage waiting for the right Tuesday.
