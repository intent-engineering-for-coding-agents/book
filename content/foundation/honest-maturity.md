...
The team is not lying. They believe they practice it. They probably did practice it for the first month after adoption. The gap between *what we believe we do* and *what the repo shows we do* is the most reliable failure mode in any engineering organisation, and *Agentic Software Engineering (ASE)* practices are no different.
...
**CI-enforced.** The pipeline fails if the practice is violated. `ase check` catches a missing `docs/README.md` before the PR can merge. A test tagged with a non-existent Acceptance Criterion ID (AC ID) fails the traceability check. CI-enforced practices are the only ones a team can rely on without trust.
...
Doc indexing has a skill that handles drift mechanically. `update-index` scans `docs/` and regenerates listing files. There is no equivalent automation for maturity claims yet. Some of this could be wired into `ase check` later, flagging any practice claimed at *CI-enforced* whose enforcing check is missing. For now it stays a human responsibility, which means it is the responsibility most likely to slip.
