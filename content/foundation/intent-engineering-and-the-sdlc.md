...
The change starts the way it always has. A ticket. A story. A Linear card. OpenSpec adds a sibling artifact, `openspec/changes/<name>/`, containing `proposal.md`, `design.md`, delta specs (one per capability under `specs/`), and `tasks.md`. The ticket is still the unit of tracking. The spec is the unit of intent.
