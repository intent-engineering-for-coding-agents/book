...
One input sits upstream of the spec itself: the architectural decision the spec executes. The chain runs ADR, then design doc, then spec ([Spec Lifecycle](../spec-driven/spec-lifecycle)), so a spec frozen against a decision still open, or one reversed in a later PR, builds on a foundation already in motion. The gate confirms the governing ADR is approved, and the design doc the spec leans on still says what the spec assumes. A link checker proves the reference resolves. Whether the decision still holds is the same manual judgment the architecture check already demands.
...
The other two inputs split along the line this chapter keeps drawing. Hooks are deterministic; the pre-commit checks [Skills, Commands, and Hooks](../agent-instructions/skills-commands-hooks) installs: lint, secret detection, AC-ID-to-tag verification, all firing unprompted. Context erodes silently, the way [Context Window Management](../agent-instructions/context-window) described: a session three hours deep, window full, and the spec buried under two unrelated tasks, writes worse code than the same session at its start. The during-gate re-teaches neither. It enforces them, and to tell that it lapsed is the agent re-deriving in hour three an import path it had right in hour one.
