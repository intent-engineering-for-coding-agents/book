...
The spec was good. The tests were proper proof. The PR landed clean. Three weeks later the on-call developer found a comment in the code referencing a design document that no longer existed. The decision the comment depended on had been silently reversed in a different PR. Nobody had checked. The implementation that passed every gate at merge time was running against an assumption that was no longer true.
...
Review is the third part of the after-checkpoint, and the order matters. Review the spec first. Then review the diff against the spec. Then review the diff on its own merits. Reversing this order is the most common mistake: the reviewer opens the diff, evaluates whether the code looks reasonable, and approves. What that approves is whether the implementation looks like reasonable code, not whether it implements what was specified. The two are not the same.
