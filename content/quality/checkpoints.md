...
Before the agent writes a line, three things have to be in place: the architecture has to be legible enough for the agent to find what it needs, `AGENTS.md` has to point at the instructions and skills the work depends on, and the design system, if there is one, has to be current.

Architecture legibility is the precondition for everything else. An agent working in a system whose modules have unclear boundaries will improvise boundaries of its own. The boundaries it improvises will not match the ones in the developers' heads. That mismatch shows up later as scope creep in PRs or as the agent placing code in the wrong layer. The fix is upstream: clearer ADRs, a current `docs/architecture/README.md` for the architecture overview, a `skeleton.md` if the codebase is brownfield.
...
Refactoring is the after-checkpoint where most teams stop. The agent generated code, the code worked, so ship it. The book's position is opposite: the agent's first generation is rarely the right shape for the next change. Refactor at the merge boundary, not in a follow-up PR three weeks later. The cost of the refactor while the spec and the implementation are both fresh is small. The cost when the next developer arrives and has to interpret unfamiliar generated code is large.
...
The three-gate model is how to spend that judgment well: use the before-gate to set up the conditions in which the agent succeeds, the during-gate to keep the work aligned, and the after-gate to confirm the alignment held. Each gate has a small number of deterministic checks and one or two human questions. Anything more is overhead.
