...
The defense is to encode the non-negotiable decisions so the question never gets asked. Certificate verification stays on, written into an agent instruction the agent reads before it touches transport code.
...
Security is not the subject of this book, and one chapter does not make it one. It has its own literature, its own specialists, and a far wider threat surface than Intent Engineering touches. The claim here is narrow: the agent introduces failure modes the standard tools were not built to see, and those particular failures yield to the same artifact discipline the rest of the book is built on.
...
None of this replaces the human who reads every PR through a security lens. The tools, rules, and pattern defenses here make that reviewer's job possible at agentic speed. They do not retire it.
