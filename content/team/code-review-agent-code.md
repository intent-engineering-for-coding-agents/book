...
This shape is this book's recommended default for behavioral changes with decision content. Many teams ship spec and code in a single PR and review the spec delta first within it. That is lighter and it works when reviewers are disciplined. The two-PR shape removes the discipline requirement: reviewing the diff before the spec is structurally harder when the spec PR has already merged.

The trigger for the split is the same one described in [Trunk-Based Development with Agents](./trunk-based-development): decision content. If an intent-level correction found during code review would force the implementation to be redone, the spec earns its own PR. If the only fix would be to the code, spec and implementation ship together.
