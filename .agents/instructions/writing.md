...
- Never use the em dash (`—`) anywhere in book content. It is an AI-writing tell. Rewrite using a comma, a colon, parentheses, or two sentences. This is a hard rule with no exceptions, including inside `*Sources:*` lines and claim-tags. See `voice.md`.
